...
- 补齐使用手册 [`doc/charts/README.md`](../../../doc/charts/README.md) 与专章 [`design/CHARTS.md`](../../../design/CHARTS.md)

## 2.5.0-20230101（2023-01-01）
- 凌云UI Charts 修改条件编译顺序，确保uniapp的cli方式的项目依赖不完整时可以正常显示
- 凌云UI Charts 恢复props属性directory的使用，以修复vue3项目中，开启echarts后，echarts目录识别错误的bug
...
- 凌云UI Charts 修复使用echarts时reload、reshow无法调用重新渲染的bug，[详见码云PR](https://gitee.com/uCharts/uCharts/pulls/40)
- 凌云UI Charts 修复使用echarts时，初始化时宽高不正确的bug，[详见码云PR](https://gitee.com/uCharts/uCharts/pulls/42)
- 凌云UI Charts 修复uniapp的h5使用history模式时，无法加载echarts的bug
- 凌云UI Charts 小程序端@complete、@scrollLeft、@scrollRight、@getTouchStart、@getTouchMove、@getTouchEnd事件增加opts参数传出，方便一些特殊需求的交互获取数据。
...
- 凌云UI Charts 修复开启canvas2d后将series赋值为空数组显示加载图标时，再次赋值后画布闪动的bug
- 凌云UI Charts 修复升级hbx最新版后ECharts的highlight方法报错的bug
...
- 凌云UI Charts 修复HBX升级3.4.6.20220420版本后echarts报错的问题
## 2.4.2-20220420（2022-04-20）
## 重要！此版本uCharts新增了很多功能，修复了诸多已知问题
- 凌云UI Charts 新增onzoom开启双指缩放功能（仅uCharts），前提需要直角坐标系类图表类型，并且ontouch为true、opts.enableScroll为true，详见实例项目K线图
- 凌云UI Charts 新增optsWatch是否监听opts变化，关闭optsWatch后，动态修改opts不会触发图表重绘
- 凌云UI Charts 修复开启canvas2d功能后，动态更新数据后画布闪动的bug
- 凌云UI Charts 去除directory属性，改为自动获取echarts.min.js路径（升级不受影响）
- 凌云UI Charts 增加getImage()方法及@getImage事件，通过ref调用getImage()方法获，触发@getImage事件获取当前画布的base64图片文件流。
- 凌云UI Charts 支付宝、字节跳动、飞书、快手小程序支持开启canvas2d同层渲染设置。
- 凌云UI Charts 新增加【非uniCloud】版本组件，避免有些不需要uniCloud的使用组件发布至小程序需要提交隐私声明问题，请到码云[【非uniCloud版本】](https://gitee.com/uCharts/uCharts/tree/master/uni-app/uCharts-%E7%BB%84%E4%BB%B6)，或npm[【非uniCloud版本】](https://www.npmjs.com/package/@qiun/uni-ucharts)下载使用。
...
- 凌云UI Charts 组件初步支持vue3，全端编译会有些问题，具体详见下面修改：
1. 小程序端运行时，在uni_modules文件夹的lingyun-ui-charts.js中搜索 new uni_modules_qiunDataCharts_js_sdk_uCharts_uCharts.uCharts，将.uCharts去掉。
2. 小程序端发行时，在uni_modules文件夹的lingyun-ui-charts.js中搜索 new e.uCharts，将.uCharts去掉，变为 new e。
...
- 凌云UI Charts 修改组件 props 属性中的 background 默认值为 rgba(0,0,0,0)
## 2.3.6-20211201（2021-12-01）
- uCharts.js 修复bar条状图开启圆角模式时，值很小时圆角渲染错误的bug
## 2.3.5-20211014（2021-10-15）
- uCharts.js 增加vue3的编译支持（仅原生uCharts，lingyun-ui-charts组件后续会支持，请关注更新）
## 2.3.4-20211012（2021-10-12）
- 凌云UI Charts 修复 mac os x 系统 mouseover 事件丢失的 bug
## 2.3.3-20210706（2021-07-06）
- uCharts.js 增加雷达图开启数据点值（opts.dataLabel）的显示
## 2.3.2-20210627（2021-06-27）
- 凌云UI Charts 修复tooltipCustom个别情况下传值不正确报错TypeError: Cannot read property 'name' of undefined的bug
...
- 凌云UI Charts 增加tapLegend属性，是否开启图例点击交互事件
- 凌云UI Charts getIndex事件中增加返回uCharts实例中的opts参数，以便在页面中调用参数
...
- 凌云UI Charts 修复小程序端开启滚动条更新数据后滚动条位置不符合预期的bug
...
- 凌云UI Charts 优化onWindowResize防抖方法
- 凌云UI Charts 修复APP端uCharts更新数据时，清空series显示loading图标后再显示图表，图表抖动的bug
- uCharts.js 修复开启canvas2d后，x轴、y轴、series自定义字体大小未按比例缩放的bug
- 示例项目 修复format-e.vue拼写错误导致app端使用uCharts渲染图表
## 2.1.3-20210513（2021-05-13）
- 凌云UI Charts 修改uCharts变更chartData数据为updateData方法，支持带滚动条的数据动态打点
- 凌云UI Charts 增加onWindowResize防抖方法 fix by ど誓言，如尘般染指流年づ 
- 凌云UI Charts H5或者APP变更chartData数据显示loading图表时，原数据闪现的bug
- 凌云UI Charts props增加errorReload禁用错误点击重新加载的方法
...
凌云UI Charts 修复APP端初始化时就传入chartData或lacaldata不显示图表的bug
## 2.1.1-20210509（2021-05-09）
- 凌云UI Charts 变更ECharts的eopts配置在renderjs内执行，支持在config-echarts.js配置文件内写function配置。
- 凌云UI Charts 修复APP端报错Prop being mutated: "onmouse"错误的bug。
- 凌云UI Charts 修复APP端报错Error: Not Found：Page[6][-1,27] at view.umd.min.js:1的bug。
## 2.1.0-20210507（2021-05-07）
- 凌云UI Charts 修复初始化时就有数据或者数据更新的时候loading加载动画闪动的bug
- uCharts.js 修复x轴format方法categories为字符串类型时返回NaN的bug
- uCharts.js 修复series.textColor、legend.fontColor未执行全局默认颜色的bug
## 2.1.0-20210506（2021-05-06）
- 凌云UI Charts 修复极个别情况下报错item.properties undefined的bug
- 凌云UI Charts 修复极个别情况下关闭加载动画reshow不起作用，无法显示图表的bug
...
- 凌云UI Charts 修复QQ小程序、百度小程序在关闭动画效果情况下，v-for循环使用图表，显示不正确的bug
## 2.0.0-20210426（2021-04-26）
- 凌云UI Charts 修复QQ小程序不支持canvas2d的bug
- 凌云UI Charts 修复钉钉小程序某些情况点击坐标计算错误的bug
...
凌云UI Charts 修复 formatterAssign 未判断 args[key] == null 的情况导致栈溢出的 bug
## 2.0.0-20210422（2021-04-22）
- 凌云UI Charts 修复H5、APP、支付宝小程序、微信小程序canvas2d模式下横屏模式的bug
## 2.0.0-20210421（2021-04-21）
- uCharts.js 修复多行图例的情况下，图例在上方或者下方时，图例float为左侧或者右侧时，第二行及以后的图例对齐方式不正确的bug
## 2.0.0-20210420（2021-04-20）
- 凌云UI Charts 修复微信小程序开启canvas2d模式后，windows版微信小程序不支持canvas2d模式的bug
- 凌云UI Charts 修改非uni_modules版本为v2.0版本lingyun-ui-charts组件
## 2.0.0-20210419（2021-04-19）
## v1.0版本已停更，建议转uni_modules版本组件方式调用，点击右侧绿色【使用HBuilderX导入插件】即可使用，示例项目请点击右侧蓝色按钮【使用HBuilderX导入示例项目】。
## 初次使用如果提示未注册&lt;lingyun-ui-charts&gt;组件，请重启HBuilderX，如仍不好用，请重启电脑；
...
- 凌云UI Charts 增加directory配置，修复H5端history模式下如果发布到二级目录无法正确加载echarts.min.js的bug
...
## [DEMO演示及在线生成工具（v2.0文档）https://demo.ucharts.cn](https://demo.ucharts.cn)
## [图表组件在项目中的应用参见 UReport数据报表](https://ext.dcloud.net.cn/plugin?id=4651) 
- 凌云UI Charts 修复APP端某些情况下报错`Not Found Page`的bug，fix by 高级bug开发技术员
- 示例项目 修复APP端v-for循环某些情况下报错`Not Found Page`的bug，fix by 高级bug开发技术员
- uCharts.js 修复非直角坐标系tooltip提示窗右侧超出未变换方向显示的bug
## 2.0.0-20210415（2021-04-15）
- 凌云UI Charts 修复H5端发布到二级目录下echarts无法加载的bug
- 凌云UI Charts 修复某些情况下echarts.off('finished')移除监听事件报错的bug
...
## [DEMO演示及在线生成工具（v2.0文档）https://demo.ucharts.cn](https://demo.ucharts.cn)
## [图表组件在项目中的应用参见 UReport数据报表](https://ext.dcloud.net.cn/plugin?id=4651) 
- 凌云UI Charts 修复H5端在cli项目下ECharts引用地址错误的bug
- 示例项目 增加ECharts的formatter用法的示例(详见示例项目format-e.vue)
- uCharts.js 增加圆环图中心背景色的配置extra.ring.centerColor
- uCharts.js 修复微信小程序安卓端柱状图开启透明色后显示不正确的bug
## 2.0.0-20210413（2021-04-13）
- 凌云UI Charts 修复百度小程序多个图表真机未能正确获取根元素dom尺寸的bug
- 凌云UI Charts 修复百度小程序横屏模式方向不正确的bug
- 凌云UI Charts 修改ontouch时，@getTouchStart@getTouchMove@getTouchEnd的触发条件
- uCharts.js 修复饼图类数据格式series属性不生效的bug
- uCharts.js 增加时序区域图 详见示例项目中ucharts.vue
## 2.0.0-20210412-2（2021-04-12）
## v1.0版本已停更，建议转uni_modules版本组件方式调用，点击右侧绿色【使用HBuilderX导入插件】即可使用，示例项目请点击右侧蓝色按钮【使用HBuilderX导入示例项目】。
## 初次使用如果提示未注册&lt;lingyun-ui-charts&gt;组件，请重启HBuilderX。如仍不好用，请重启电脑，此问题已于DCloud官方确认，HBuilderX下个版本会修复。
## [DEMO演示及在线生成工具（v2.0文档）https://demo.ucharts.cn](https://demo.ucharts.cn)
## [图表组件在uniCloudAdmin中的应用 UReport数据报表](https://ext.dcloud.net.cn/plugin?id=4651) 
- 凌云UI Charts 修复uCharts在APP端横屏模式下不能正确渲染的bug
- 示例项目 增加ECharts柱状图渐变色、圆角柱状图、横向柱状图（条状图）的示例
## 2.0.0-20210412（2021-04-12）
- 凌云UI Charts 修复created中判断echarts导致APP端无法识别，改回mounted中判断echarts初始化
- uCharts.js 修复2d模式下series.textOffset未乘像素比的bug
## 2.0.0-20210411（2021-04-11）
## v1.0版本已停更，建议转uni_modules版本组件方式调用，点击右侧绿色【使用HBuilderX导入插件】即可使用，示例项目请点击右侧蓝色按钮【使用HBuilderX导入示例项目】。
## 初次使用如果提示未注册<lingyun-ui-charts>组件，请重启HBuilderX，并清空小程序开发者工具缓存。
## [DEMO演示及在线生成工具（v2.0文档）https://demo.ucharts.cn](https://demo.ucharts.cn)
## [图表组件在uniCloudAdmin中的应用 UReport数据报表](https://ext.dcloud.net.cn/plugin?id=4651) 
- uCharts.js 折线图区域图增加connectNulls断点续连的功能，详见示例项目中ucharts.vue
- 凌云UI Charts 变更初始化方法为created，变更type2d默认值为true，优化2d模式下组件初始化后dom获取不到的bug
- 凌云UI Charts 修复左右布局时，右侧图表点击坐标错误的bug，修复tooltip柱状图自定义颜色显示object的bug
...
# 凌云UI Charts+uCharts v2.0版本同步上线，使用方法详见https://demo.ucharts.cn帮助页
## 2.0.0（2021-04-05）
# 凌云UI Charts+uCharts v2.0版本同步上线，使用方法详见https://demo.ucharts.cn帮助页
